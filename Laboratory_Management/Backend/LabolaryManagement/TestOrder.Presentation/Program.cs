...
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='Booking' AND column_name='VoucherId') THEN
        ALTER TABLE ""Booking"" ADD COLUMN ""VoucherId"" INT;
        ALTER TABLE ""Booking"" ADD CONSTRAINT ""FK_Booking_Voucher"" FOREIGN KEY (""VoucherId"") REFERENCES ""Voucher""(""VoucherId"") ON DELETE SET NULL;
    END IF;
END $$;
");
                db.Database.ExecuteSqlRaw(@"
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='Booking' AND column_name='DiscountAmount') THEN
        ALTER TABLE ""Booking"" ADD COLUMN ""DiscountAmount"" DOUBLE PRECISION;
    END IF;
END $$;
");
                Console.WriteLine("✅ TestOrder database schema ensured & Voucher schema migrated");
            }
